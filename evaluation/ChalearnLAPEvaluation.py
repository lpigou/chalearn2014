...
def gesture_overlap_csv(csvpathgt, csvpathpred, seqlenght):
    ...
    # falsePos=numpy.setdiff1d(predGestures, gtGestures) 
    err=numpy.setdiff1d(gtGestures,predGestures)
    global fps, errs, totpred, totgt
    fps += len(numpy.setdiff1d(predGestures, gtGestures) )
    errs += len(err)
    totpred += len(predGestures)
    totgt += len(gtGestures)
...
def evalGesture(prediction_dir,truth_dir):
    ...
    print "falsepos",fps,"(% ",100.*fps/totpred, "), errors",errs,"(%",100.*errs/totgt, "), totgt",totgt, ", totpred",totpred,score/numSamples
    return score/numSamples
