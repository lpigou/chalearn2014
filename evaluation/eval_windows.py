...
# sys.path.append('..')
import eval_model_windows as eval_model
...
import ChalearnLAPSample_windows as ChalearnLAPSample
...
def eval():
    ...
    for fileName in files:
        ...
        # s_time = time.time()
        smp = ChalearnLAPSample.GestureSample(fileName)
...
if __name__ == '__main__': 
    ...
    # files = glob(data+"/"+"*.zip")
    # files.sort()
    # p_ = Process(target=eval, args=(q,))
    # # p_.daemon=True
    # p_.start()
    # p2 = Process(target=write_loop, args=(q2,))
    # # p2.daemon=True
    # p2.start()
    # pred_loop()
    # p_.terminate()
