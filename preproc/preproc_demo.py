...
def main(args):
    ...
    #start preprocessing
    samples = list(array(samples)[[294,339,75,263,433]])
    preprocess(samples)
...
def preprocess(samples):
    ...
    for file in samples:
        ...
        for gesture in gestures[:5]:
            ...
            def resize(v, size, interp=cv2.INTER_NEAREST): # _NEAREST _LINEAR _AREA _CUBIC
            ...
            if first:
                ...
                vsk = VideoSink('/home/lio/Desktop/video.avi',rate=20, 
                    size=vid.shape[1:3],colorspace='rgb24', codec="x264")
                first = False
...
def play_vid(vid, wait=1000/20, norm=True, fs=False, resize=True):
    if isinstance(vid, list):
        for img0,img1 in zip(vid[0],vid[1]): 
            show_imgs([img0,img1], wait, norm)
    elif vid.ndim == 4:
        vid = vid.swapaxes(0,1)
        for imgs in vid: show_imgs(list(imgs), wait, norm, fs, resize)
    else: 
        for img in vid: show_img(img, wait, norm, fs,resize)
     

def show_imgs(imgs, wait=0, norm=True, fs=False, resize=True):
    if norm:
        for i, img in enumerate(imgs):
            imgs[i] = norm_img(img)

    img = hstack(imgs)
    show_img(img, wait, False, fs, resize)
