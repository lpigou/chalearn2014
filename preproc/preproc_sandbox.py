...
#settings
data = "E:\\mp\\chalearn2014\\train_raw"
# data = "/media/Data/mp/chalearn2014/train_raw"
# dest = "/media/Data/mp/chalearn2014/40x90x90_train_21lbls"
# data = "/media/lio/64EE5F7C8CC54BFB/chalearn2014/train"
# dest = "/media/lio/64EE5F7C8CC54BFB/chalearn2014/20lbl_32x128x128/valid"
...
def main(args):
    ...
    #start preprocessing
    # samples = list(array(samples)[[339,75,263,75,433,294]])
    preprocess(samples[333:]) #333 440

def proc_gray2(gray, skelet):
    traj2D,traj3D,ori,pheight,hand,center = skelet
    offset = 50
    vid = gray.copy()
    for i,img in enumerate(vid):
        center2 = traj2D[:,i]
        c = center2.round().astype("int")

        x = (c[0]-offset, c[0]+offset)
        y = (c[1]-offset, c[1]+offset)
        x,y = fit_screen(x,y)

        cv2.rectangle(img, (x[0],y[0]), (x[1],y[1]), (255,255,255),3)
        ...
        y = int(round(c[1,i]+h*0.5))
        l = int(round(h*1.3 + (y-c[1,i])))
    ...
    # play_vid(vid, norm=False, wait=0)
    return vid
...
def save_img(img,name,size):
    img = cv2.resize(img,size, interpolation=cv2.INTER_AREA)

    cv2.imwrite("C:/Users/lio/Desktop/pres/preproc/"+name+".png", img,
...
def proc_depth(depth, user, user_o, skelet):
    ...
    # play_vid(depth, norm=False, wait=0)
    fr = depth.shape[0]/2
    ...
    #stats
    depth = 255 - depth
    # show_img(depth[fr],resize=False)
    ...
    # show_img(depth_b[fr],resize=False)
    # show_img(depth_h[fr],resize=False)
    # show_img(user_b[fr],resize=False)
    # show_img(user_h[fr],resize=False)
    ...
    for i,(depth,user) in enumerate(((depth_b,user_b),(depth_h,user_h))):
        ...
        # show_img(user[fr],norm=True,resize=True)
        # show_img(depth[fr],resize=True)
        ...
        # show_img(clip(depth[fr], 0, 255),resize=True)
...
def show_img(img, wait=0, norm=False, fs=False, resize=True):
