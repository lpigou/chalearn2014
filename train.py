...
"""
try_doc = "20l variance 2014"
number_of_classes = 20
inspect = False
print try_doc

# own imports
from data_aug import start_load, load_normal, augment
...
if pc=="laptop":
    ...
    src = "/media/lio/64EE5F7C8CC54BFB/chalearn2013/21lbl_32x128x128" # dir of preprocessed data
...
# _files = glob(src+'/train/batch_100_*.zip')+glob(src+'/valid/batch_100_*.zip')
# rng.shuffle(_files)
class files:
...
for stage in xrange(n_stages):
    for i in xrange(len(out)): # for each convnet of the stage
        ...
        if stage==0:
            ...
            # out[i] = NormLayer(out[i], method="lcn", kernel_size=9,
            #     use_divisor=use.norm_div).output
