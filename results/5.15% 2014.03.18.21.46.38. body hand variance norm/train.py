...
scaler = 127.5
scaler_traj = 10
try_doc = "body hand traj variance norm"
print try_doc
...
class net:
    ...
    # maps = [1,32,64,128] # feature maps in each convolutional network
    maps = [2,5,25,25] # feature maps in each convolutional network
    kernels = [(1,7,7), (1,7,7), (1,6,6)] # convolution kernel shapes
...
for stage in xrange(n_stages):
    for i in xrange(len(out)): # for each convnet of the stage
        # normalization
        if use.norm and stage>0: out[i] = NormLayer(out[i], method=net.norm_method,
            use_divisor=use.norm_div).output
        out[i] = var_norm(out[i])
        # convolutions  
...
elif use.traj: traj_ = var_norm(t.flatten(2))
