...
def load_data(path, trans): 
    ...
    vid, traj, lbl = vid[ind].astype(floatX), (traj[ind]/255.).astype(floatX), (lbl[ind]-1).astype(floatX)
...
for stage in xrange(n_stages):
    for i in xrange(len(out)): # for each convnet of the stage
        ...
        # normalization
        if use.norm: out[i] = NormLayer(out[i], 
...
if use.trajconv:
    ...
    for i in xrange(trajconv.layers):
        ...
        t_b.append(ones((n_out,), dtype=floatX)*0.1)
        t_conv += t_b.dimshuffle('x',0)
        t_conv = activation(t_conv)

    conv_length = prod(traj_shape[1:-1])*trajconv.res_shape
    t_conv = t_conv.reshape((batch.micro, conv_length))
    if trajconv.append: traj_ = T.concatenate([t.flatten(2), t_conv], axis=1)
...
if net.fusion == "early":
    ...
    n_in_MLP -= net.maps[-1]*net.n_convnets*prod(video_shapes[-1])
    layers.append(HiddenLayer(traj_, 
        n_in=n_in_MLP, 
        n_out=net.hidden_traj, 
        rng=rng, 
        activation=activation))
    n_in_MLP = net.maps[-1]*net.n_convnets*prod(video_shapes[-1])
    layers.append(HiddenLayer(vid_, 
        n_in=n_in_MLP, 
...
if use.trajconv: params.extend([t_W, t_b])
# gradient list
...
def training_report(train_ce):
    # for par in params:
    #     p = par.get_value(borrow=True)
    #     print p.shape, p.min(), p.mean(), p.max()
...
def save_results(train_ce, valid_ce, valid2_ce=None):
    dump(params, open(res_dir+"/params.p", "wb"), -1)
...
def _mini_batch(model, mini_batch, is_train):
    ce = []
    for i in xrange(batch.mini/batch.micro): ce.append(model(mini_batch, i))
