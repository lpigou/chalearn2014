...
if pc=="laptop":
    ...
    src = "/home/lio/mp/chalearn2014/40x90x90_train_21lbls" # dir of preprocessed data
    res_dir_ = '/home/lio/Dropbox/MP/chalearn2014/results' # dir to print and store results
...
class batch:
    ...
    micro = 5 if pc=="reslab" else 2 # number of samples that fits in memory
...
class files:
    ...
    valid = glob(src+'/valid/batch_100_*.zip')
    n_valid = len(valid)
...
def load_data(path, trans): 
    ...
    if first_report:
        print "vid range:",vid.min(),vid.max()
...
layers.append(LogRegr(out, rng=rng, activation=activation, n_in=net.hidden, 
    W_scale=net.W_scale[-1], b_scale=net.b_scale, n_out=21))
